...
import { styles } from '../bankapp/bankapp.style'
import ArrowRightAltIcon from '@mui/icons-material/ArrowRightAlt';
import CurrencyRupeeIcon from '@mui/icons-material/CurrencyRupee';
import User from "../users.json"
import img from "../bankapp/Screenshot from 2023-11-17 11-31-20.png"
        ...
        if(data<=loandata.balance){
            const req=array.map((item,index)=>{
                if(item.user==uname){
                  ...
                  // alert("10% minimum balance required")
                    return item;
                }
                    ...
                    <Box sx={styles.transection}>
                        <Box sx={styles.trs1}>
                            <Paper elevation={9} sx={styles.parer1}>
                                {
                                    array.filter((item)=>(
                                        item.user===uname  && item.pin===password 
                                    )).map((element, index) => {
                                        return (
                                            <Box>
                                                <Paper sx={styles.deposit} elevation={5}>
                                                    ...
                                                    <Typography sx={styles.whd}>{element.deposit}</Typography>
                                                </Paper>
                                                <Paper sx={styles.withdraw} elevation={5}>
                                                    <Typography sx={styles.with}>Withdraw</Typography>
                                                    <Typography sx={styles.date}>{date}</Typography>
                                                    <Typography sx={styles.whd}>{element.withdraw}</Typography>
                                                </Paper>
